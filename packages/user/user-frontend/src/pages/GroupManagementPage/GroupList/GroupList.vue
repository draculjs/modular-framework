<template>
  <v-row row wrap>
    ...
    <v-col cols="12">
      ...
      >
        <div slot="no-data" color="info" outline class="text-xs-center">
          Sin datos
        </div>
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      ...
      try {
        const groups = await GroupProvider.paginateGroups(
          this.itemsPerPage,
          this.pageNumber,
          this.search,
          this.getOrderBy,
        ...
        this.items = groups.data.groupsPaginate.items
        this.totalItems = groups.data.groupsPaginate.totalItems
      } catch (error) {
        console.error(error)
      } finally {
        this.loading = false
