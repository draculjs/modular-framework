...
<script>
    ...
    export default {
        ...
        components: {AuditShowData, CrudDelete},
        ...
        methods: {
            remove() {
                this.loading = true
                AuditProvider.deleteAudit(this.item.id).then(result => {
                            if (result.data.deleteAudit.success) {
                                this.$emit('itemDeleted',result.data.deleteAudit)
                                this.$emit('close')
                            }else{
