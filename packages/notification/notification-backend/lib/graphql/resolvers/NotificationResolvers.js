  ...
  Mutation: {
    markAsReadOrNotRead: function markAsReadOrNotRead(_, _ref7, _ref8) {
      ...
      var user = _ref8.user,
          rbac = _ref8.rbac;
      if (!user) throw new _apolloServerExpress.AuthenticationError("Unauthenticated");
      if (!rbac.isAllowed(user.id, _permissions.NOTIFICATION_UPDATE)) throw new _apolloServerExpress.ForbiddenError("Not Authorized");
    ...
    createNotification: function createNotification(_, _ref11, _ref12) {
      ...
      var user = _ref12.user,
          rbac = _ref12.rbac;
      if (!user) throw new _apolloServerExpress.AuthenticationError("Unauthenticated");
      if (!rbac.isAllowed(user.id, _permissions.NOTIFICATION_CREATE)) throw new _apolloServerExpress.ForbiddenError("Not Authorized");
