<template>
    <v-row>
        <v-col cols="12" sm="6" md="4">
            <v-list>
                 <show-field :value="getDateTimeFormat(item.blockedUntil)" :label="$t('queue.queue.labels.blockedUntil')" icon="event"/>
                 <show-field :value="String(item.retries)" :label="$t('queue.queue.labels.retries')" icon="double_arrow"/>
        ...
        <v-col cols="12" sm="6" md="4">
            <v-list>
                 <show-field :value="String(item.maxRetries)" :label="$t('queue.queue.labels.maxRetries')" icon="last_page"/>
                 <show-field :value="item.progressDetail" :label="$t('queue.queue.labels.progressDetail')" icon="info"/>
                 <show-field :value="item.payload" :label="$t('queue.queue.labels.payload')" icon="note_add"/>
            </v-list>
        </v-col>
