...
class WorkerManager {
  ...
  processJob(subscription) {
    return new Promise((resolve, reject) => {
      ...
      subscription.worker.get(subscription.topic).then(job => {
        //Si obtuve un Job de la Queue
        if (job) {
          //El handler procesa el job
          subscription.handler(job).then(result => {
            if (result) {
              console.log("the job " + job.id + " with topic " + job.topic + " was completed successfully"); //Notifico a la queue que el job se completo con exito

              subscription.worker.ack(job.id).then(rjob => {
